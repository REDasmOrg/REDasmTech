...
const char* dexProtoGetMethodDescriptor(const DexProto* pProto,
    ...
    dex_u4 paramCount = (typeList == NULL) ? 0 : typeList->size;
    dex_u4 i;

    for (i = 0; i < paramCount; i++) {
        dex_u4 idx = dexTypeListGetIdx(typeList, i);
...
static int protoCompare(const DexProto* pProto1, const DexProto* pProto2,
    ...
    if (pProto1 == pProto2) {
        ...
        for (i = 0; i < minParam; i++) {
            dex_u4 idx1 = dexTypeListGetIdx(typeList1, i);
            dex_u4 idx2 = dexTypeListGetIdx(typeList2, i);
            int result =
 ...
 */
dex_u4 dexParameterIteratorNextIndex(DexParameterIterator* pIterator) {
    ...
    if (cursor >= parameterCount) {
        ...
        dex_u4 idx = dexTypeListGetIdx(pIterator->parameters, cursor);
        pIterator->cursor++;
        return idx;
    }
}
...
const char* dexParameterIteratorNextDescriptor(
    ...
    dex_u4 idx = dexParameterIteratorNextIndex(pIterator);
