 ...
 */
static inline bool checkPtrRange(const CheckState* state, const void* start,
                                 const void* end, const char* label) {
    const void* fileStart = state->fileStart;
    const void* fileEnd = state->fileEnd;
    if((start < fileStart) || (start > fileEnd) || (end < start) ||
       (end > fileEnd)) {
...
#define CHECK_OFFSET_RANGE(_start, _end)                                       \
    {                                                                          \
        ...
        if(!checkPtrRange(state, _startPtr, _endPtr, #_start ".." #_end)) {    \
            return 0;                                                          \
        }                                                                      \
    }
...
#define CHECK_INDEX(_field, _limit)                                            \
    {                                                                          \
        if((_field) >= (_limit)) {                                             \
            ALOGW("Bad index: %s(%u) > %s(%u)", #_field, (dex_u4)(_field),     \
                  #_limit, (dex_u4)(_limit));                                  \
            return 0;                                                          \
        }                                                                      \
    }
...
//     }
//
//     const DexMapList* pMap = (const DexMapList*) filePointer(state,
//     sectionOffset);
//
//     *endOffset =
 ...
 * so, false if not. */
static bool shortyDescMatch(char shorty, const char* descriptor,
                            bool isReturnType) {
    switch(shorty) {
        case 'V': {
            if(!isReturnType) {
        ...
        case 'Z': {
            if((descriptor[0] != shorty) || (descriptor[1] != '\0')) {
                ALOGE("Shorty vs. primitive type mismatch: '%c', '%s'", shorty,
                      descriptor);
                return false;
            }
            break;
        ...
        case 'L': {
            if((descriptor[0] != 'L') && (descriptor[0] != '[')) {
                ALOGE("Shorty vs. type mismatch: '%c', '%s'", shorty,
                      descriptor);
                return false;
            }
            break;
...
static dex_u4
findFirstAnnotationsDirectoryDefiner(const CheckState* state,
...
//         item->superclassIdx); if (!dexIsClassDescriptor(descriptor)) {
...
//         item->callSiteOff >= state->pHeader->dataOff +
//         state->pHeader->dataSize) { ALOGE("Bad call site offset: %u",
//         item->callSiteOff); return NULL;
//     }
//     return (void*) (item + 1);
// }
...
//                 item->fieldOrMethodIdx); return NULL;
//             }
//             break;
...
// static dex_u1* swapFieldAnnotations(const CheckState* state, dex_u4 count,
// dex_u1* addr) {
...
// static dex_u1* swapMethodAnnotations(const CheckState* state, dex_u4 count,
// dex_u1* addr) {
...
// static dex_u1* swapParameterAnnotations(const CheckState* state, dex_u4
// count,
//         dex_u1* addr) {
 ...
 * field elements. */
// static const dex_u1* crossVerifyFieldAnnotations(const CheckState* state,
// dex_u4 count,
...
//         void* ptr) {
//     const DexAnnotationsDirectoryItem* item = (const
//     DexAnnotationsDirectoryItem*) ptr; dex_u4 definingClass =
//     findFirstAnnotationsDirectoryDefiner(state, item);
//
//     if (!dexDataMapVerify0Ok(state->pDataMap,
...
//         void* ptr) {
//     const DexAnnotationSetRefList* list = (const DexAnnotationSetRefList*)
//     ptr; const DexAnnotationSetRefItem* item = list->list; int count =
//     list->size;
//
//     while (count--) {
//         if (!dexDataMapVerify0Ok(state->pDataMap,
...
static bool verifyFields(const CheckState* state, dex_u4 size, DexField* fields,
                         bool expectStatic) {
    dex_u4 i;
...
static bool verifyMethods(const CheckState* state, dex_u4 size,
    ...
    for(i = 0; i < size; i++) {
        ...
        if(isSynchronized && !allowSynchronized) {
            ALOGE("Bogus method access flags (synchronization) %x @ %d",
                  accessFlags, i);
            return false;
        }
...
//     DexClassData* classData = dexReadAndVerifyClassData(&data,
//     state->fileEnd);
//
//     if (classData == NULL) {
...
//     const dex_u1* data = (const dex_u1*) ptr;
//     DexClassData* classData = dexReadAndVerifyClassData(&data,
//     state->fileEnd); dex_u4 definingClass = findFirstClassDataDefiner(state,
//     classData); bool okay = true; dex_u4 i;
//
...
//         DexCode* code, dex_u4 firstOffset, dex_u4 handlersSize, dex_u4*
//         handlerOffs) {
...
//                 dex_u4 idx;
//                 dex_u4 regNum = readAndVerifyUnsignedLeb128(&data, fileEnd,
//                 &okay); if (!okay) break; if (regNum >= 65536) {
//                     okay = false;
//                     break;
//                 }
...
//             }
//             case DBG_SET_FILE: {
//                 dex_u4 idx = readAndVerifyUnsignedLeb128(&data, fileEnd,
//                 &okay); if (!okay) break; if (idx != 0) {
//                     idx--;
//                     CHECK_INDEX(idx, state->pHeader->stringIdsSize);
//                 }
//                 break;
 ...
 * little endian value. */
static dex_u4 readUnsignedLittleEndian(const CheckState* state,
                                       const dex_u1** pData, dex_u4 size) {
...
//         dex_u4 offset, dex_u4 count, ItemVisitorFunction* func, dex_u4
//         alignment, dex_u4* nextOffset, int mapType) {
...
//         dex_u4 offset, dex_u4 count, dex_u4 expectedOffset, dex_u4
//         expectedCount, ItemVisitorFunction* func, dex_u4 alignment, dex_u4*
//         nextOffset) {
 ...
 */
// static bool iterateDataSection(CheckState* state, dex_u4 offset, dex_u4
// count,
//         ItemVisitorFunction* func, dex_u4 alignment, dex_u4* nextOffset, int
//         mapType) {
...
//             const dex_u1* ptr = (const dex_u1*) filePointer(state,
//             lastOffset); while (lastOffset < sectionOffset) {
//                 if (*ptr != '\0') {
//                     ALOGE("Non-zero padding 0x%02x before section start @
//                     %x",
//                             *ptr, lastOffset);
//                     okay = false;
//                     break;
...
//                 okay = iterateDataSection(state, sectionOffset, sectionCount,
//                         swapAnnotationSetRefList, sizeof(dex_u4),
//                         &lastOffset, type);
//                 break;
//             }
//             case kDexTypeAnnotationSetItem: {
//                 okay = iterateDataSection(state, sectionOffset, sectionCount,
//                         swapAnnotationSetItem, sizeof(dex_u4), &lastOffset,
//                         type);
//                 break;
//             }
//             case kDexTypeClassDataItem: {
//                 okay = iterateDataSection(state, sectionOffset, sectionCount,
//                         intraVerifyClassDataItem, sizeof(dex_u1),
//                         &lastOffset, type);
//                 break;
//             }
...
//                 okay = iterateDataSection(state, sectionOffset, sectionCount,
//                         intraVerifyStringDataItem, sizeof(dex_u1),
//                         &lastOffset, type);
//                 break;
//             }
//             case kDexTypeDebugInfoItem: {
//                 okay = iterateDataSection(state, sectionOffset, sectionCount,
//                         intraVerifyDebugInfoItem, sizeof(dex_u1),
//                         &lastOffset, type);
//                 break;
//             }
//             case kDexTypeAnnotationItem: {
//                 okay = iterateDataSection(state, sectionOffset, sectionCount,
//                         intraVerifyAnnotationItem, sizeof(dex_u1),
//                         &lastOffset, type);
//                 break;
//             }
//             case kDexTypeEncodedArrayItem: {
//                 okay = iterateDataSection(state, sectionOffset, sectionCount,
//                         intraVerifyEncodedArrayItem, sizeof(dex_u1),
//                         &lastOffset, type);
//                 break;
//             }
//             case kDexTypeAnnotationsDirectoryItem: {
//                 okay = iterateDataSection(state, sectionOffset, sectionCount,
//                         swapAnnotationsDirectoryItem, sizeof(dex_u4),
//                         &lastOffset, type);
//                 break;
//             }
//             default: {
...
//             case kDexTypeAnnotationSetRefList: {
//                 okay = iterateSection(state, sectionOffset, sectionCount,
//                         crossVerifyAnnotationSetRefList, sizeof(dex_u4),
//                         NULL);
//                 break;
//             }
//             case kDexTypeAnnotationSetItem: {
...
//             case kDexTypeAnnotationsDirectoryItem: {
//                 okay = iterateSection(state, sectionOffset, sectionCount,
//                         crossVerifyAnnotationsDirectoryItem, sizeof(dex_u4),
//                         NULL);
//                 break;
//             }
//             default: {
...
//         if (len != expectedLen) {
//             ALOGE("ERROR: Bad length: expected %u, got %zu", expectedLen,
//             len); okay = false;
//         }
//     }
//
...
//          */
//         const int nonSum = sizeof(pHeader->magic) +
//         sizeof(pHeader->checksum); dex_u4 storedFileSize =
//         SWAP4(pHeader->fileSize); dex_u4 expectedChecksum =
//         SWAP4(pHeader->checksum); dex_u4 adler = RD_Adler32(((const dex_u1*)
//         pHeader) + nonSum, storedFileSize - nonSum);
//
