 ...
 */
DEX_INLINE int readUnsignedLeb128(const dex_u1** pStream) {
    const dex_u1* ptr = *pStream;
    int result = *(ptr++);
 ...
 * non-zero high-order bits in the fifth encoded byte.
 */
DEX_INLINE int readSignedLeb128(const dex_u1** pStream) {
    const dex_u1* ptr = *pStream;
    int result = *(ptr++);
 ...
 */
int readAndVerifyUnsignedLeb128(const dex_u1** pStream, const dex_u1* limit,
        bool* okay);
 ...
 * then the read is considered invalid.
 */
int readAndVerifySignedLeb128(const dex_u1** pStream, const dex_u1* limit, bool* okay);
 ...
 */
DEX_INLINE dex_u1* writeUnsignedLeb128(dex_u1* ptr, dex_u4 data)
{
    while (true) {
        dex_u1 out = data & 0x7f;
        if (out != data) {
            *ptr++ = out | 0x80;
            data >>= 7;
        } else {
 ...
 */
DEX_INLINE int unsignedLeb128Size(dex_u4 data)
{
    int count = 0;
