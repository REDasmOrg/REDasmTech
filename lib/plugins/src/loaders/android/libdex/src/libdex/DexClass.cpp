 ...
 * of uleb128 values. */
static bool verifyUlebs(const dex_u1* pData, const dex_u1* pLimit, dex_u4 count) {
    bool okay = true;
 ...
 * only; it does not verify that access flags, indices, or offsets
 * are valid. */
DexClassData* dexReadAndVerifyClassData(const dex_u1** pData, const dex_u1* pLimit) {
    ...
    dex_u1* ptr = ((dex_u1*) result) + sizeof(DexClassData);
    bool okay = true;
    dex_u4 i;
