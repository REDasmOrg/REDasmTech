 ...
 */
typedef dex_u1 InstructionWidth;

/*
 * Opcode control flow flags, used by the verifier and JIT.
 */
typedef dex_u1 OpcodeFlags;
...
struct InstructionInfoTables {
    dex_u1*                formats;    /* InstructionFormat elements */
    dex_u1*                indexTypes; /* InstructionIndexType elements */
    OpcodeFlags*       flags;
...
struct DecodedInstruction {
    dex_u4      vA;
    dex_u4      vB;
    dex_u8      vB_wide;        /* for kFmt51l */
    dex_u4      vC;
    dex_u4      arg[5];         /* vC/D/E/F/G in invoke or filled-new-array */
    Opcode  opcode;
...
{
    assert((dex_u4) opcode < kNumPackedOpcodes);
