...
struct DexCatchHandler {
    dex_u4          typeIdx;    /* type index of the caught exception type */
    dex_u4          address;    /* handler address */
};
...
dex_u4 dexGetFirstHandlerOffset(const DexCode* pCode);

/* Get count of handler lists for the given DexCode. */
dex_u4 dexGetHandlersSize(const DexCode* pCode);
...
struct DexCatchIterator {
    const dex_u1* pEncodedData;
    bool catchesAll;
    dex_u4 countRemaining;
...
DEX_INLINE void dexCatchIteratorInitToPointer(DexCatchIterator* pIterator,
    const dex_u1* pEncodedData)
{
    dex_s4 count = readSignedLeb128(&pEncodedData);
...
DEX_INLINE void dexCatchIteratorInit(DexCatchIterator* pIterator,
    const DexCode* pCode, dex_u4 offset)
{
...
DEX_INLINE DexCatchHandler* dexCatchIteratorNext(DexCatchIterator* pIterator) {
    if (pIterator->countRemaining == 0) {
        ...
        dex_u4 typeIdx = readUnsignedLeb128(&pIterator->pEncodedData);
...
dex_u4 dexCatchIteratorGetEndOffset(DexCatchIterator* pIterator,
    const DexCode* pCode);

/* Helper for dexFindCatchHandler(). Do not call directly. */
int dexFindCatchHandlerOffset0(dex_u2 triesSize, const DexTry* pTries,
        dex_u4 address);
...
DEX_INLINE bool dexFindCatchHandler(DexCatchIterator *pIterator,
        const DexCode* pCode, dex_u4 address) {
    dex_u2 triesSize = pCode->triesSize;
    int offset = -1;
    ...
    switch (triesSize) {
        ...
        case 1: {
            ...
            dex_u4 start = tries[0].startAddr;

            if (address < start) {
                break;
            }

            dex_u4 end = start + tries[0].insnCount;
