 ...
 */
typedef int (*DexDebugNewPositionCb)(void *cnxt, dex_u4 address, dex_u4 lineNum);

/*
 * Callback for "new locals table entry". "signature" is an empty string
 * if no signature is available for an entry.
 */
typedef void (*DexDebugNewLocalCb)(void *cnxt, dex_u2 reg, dex_u4 startAddress,
        dex_u4 endAddress, const char *name, const char *descriptor,
