...
struct DexClassDataHeader {
    dex_u4 staticFieldsSize;
    dex_u4 instanceFieldsSize;
    dex_u4 directMethodsSize;
    dex_u4 virtualMethodsSize;
};

/* expanded form of encoded_field */
struct DexField {
    dex_u4 fieldIdx; /* index to a field_id_item */
    dex_u4 accessFlags;
};

/* expanded form of encoded_method */
struct DexMethod {
    dex_u4 methodIdx; /* index to a method_id_item */
    dex_u4 accessFlags;
    dex_u4 codeOff; /* file offset to a code_item */
};
...
bool dexReadAndVerifyClassDataHeader(const dex_u1** pData, const dex_u1* pLimit,
 ...
 * are valid. */
bool dexReadAndVerifyClassDataField(const dex_u1** pData, const dex_u1* pLimit,
                                    DexField* pField, dex_u4* lastIndex);
 ...
 * are valid. */
bool dexReadAndVerifyClassDataMethod(const dex_u1** pData, const dex_u1* pLimit,
                                     DexMethod* pMethod, dex_u4* lastIndex);
 ...
 * only; it does not verify that access flags, indices, or offsets
 * are valid. */
DexClassData* dexReadAndVerifyClassData(const dex_u1** pData,
                                        const dex_u1* pLimit);
...
DEX_INLINE const DexCode* dexGetCode(const DexFile* pDexFile,
    ...
    if(pDexMethod->codeOff == 0) return NULL;
 ...
 * data. */
DEX_INLINE void dexReadClassDataHeader(const dex_u1** pData,
                                       DexClassDataHeader* pHeader) {
 ...
 * decode process.
 */
DEX_INLINE void dexReadClassDataField(const dex_u1** pData, DexField* pField,
                                      dex_u4* lastIndex) {
    dex_u4 index = *lastIndex + readUnsignedLeb128(pData);
 ...
 * decode process.
 */
DEX_INLINE void dexReadClassDataMethod(const dex_u1** pData, DexMethod* pMethod,
                                       dex_u4* lastIndex) {
    dex_u4 index = *lastIndex + readUnsignedLeb128(pData);
