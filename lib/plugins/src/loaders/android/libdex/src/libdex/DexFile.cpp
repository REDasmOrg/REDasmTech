 ...
 */
static const char* dexSHA1DigestToStr(const unsigned char digest[],
                                      char* tmpBuf) {
...
static void classLookupAdd(DexFile* pDexFile, DexClassLookup* pLookup,
    ...
    const char* classDescriptor = (const char*)(pDexFile->baseAddr + stringOff);
...
DexClassLookup* dexCreateClassLookup(DexFile* pDexFile) {
    ...
    for(i = 0; i < (int)pDexFile->pHeader->classDefsSize; i++) {
        ...
        classLookupAdd(pDexFile, pLookup, (dex_u1*)pString - pDexFile->baseAddr,
                       (dex_u1*)pClassDef - pDexFile->baseAddr, &numProbes);

        if(numProbes > maxProbes) maxProbes = numProbes;
    ...
    ALOGV("Class lookup: classes=%d slots=%d (%d%% occ) alloc=%d"
          ...
          (100 * pDexFile->pHeader->classDefsSize) / numEntries, allocSize,
...
DexFile* dexFileParse(const dex_u1* data, size_t length, int flags) {
    ...
    if(pDexFile == NULL) goto bail; /* alloc failure */
    ...
    if(memcmp(data, DEX_OPT_MAGIC, 4) == 0) {
        ...
        if(memcmp(magic + 4, DEX_OPT_MAGIC_VERS, 4) != 0) {
            ALOGE("bad opt version (0x%02x %02x %02x %02x)", magic[4], magic[5],
                  magic[6], magic[7]);
            goto bail;
        }
        ...
        if(!dexParseOptData(data, length, pDexFile)) goto bail;
    ...
    if(flags & kDexParseVerifyChecksum) {
        ...
        if(adler != pHeader->checksum) {
            ALOGE("ERROR: bad checksum (%08x vs %08x)", adler,
                  pHeader->checksum);
            if(!(flags & kDexParseContinueOnError)) goto bail;
        }
        else {
        ...
        if(pOptHeader != NULL) {
            ...
            if(adler != pOptHeader->checksum) {
                ALOGE("ERROR: bad opt checksum (%08x vs %08x)", adler,
                      pOptHeader->checksum);
                if(!(flags & kDexParseContinueOnError)) goto bail;
            }
            else {
    ...
    if(kVerifySignature) {
        ...
        const int nonSum =
            sizeof(pHeader->magic) + sizeof(pHeader->checksum) + kSHA1DigestLen;
